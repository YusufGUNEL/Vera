...
const Map<String, String> _ruStrings = {
    ...
    'homeFirstStepsBody': 'Vera становится заметно полезнее после появления первых данных. Импортируйте выписку, отсканируйте чек или добавьте банк, чтобы оживить главный экран.',
    'homeFirstStepsHint': 'Даже одного такого шага достаточно, чтобы появились подсказки, балансы и умные уведомления.',
    'connectedAccountsEmptyTitle': 'Пока нет подключённых счетов',
    'connectedAccountsEmptyBody': 'Когда вы добавите банк, мы сможем показывать баланс на главном экране и готовить действия Uma с более точным контекстом.',
    ...
    'umaInsightNoData': 'Vera пока вас не знает. Импортируйте выписку, отсканируйте чек или добавьте банк, чтобы мы вместе начали строить более умные подсказки.',
    'umaInsightNoTransactions': 'Пока не видно ни одной операции. Импортируйте первую выписку или добавьте операцию вручную, чтобы Uma поняла ваш ритм расходов.',
    'umaInsightNoPattern': 'Для этого периода пока нет достаточно внятного паттерна расходов. Когда появятся новые операции, инсайт здесь станет точнее.',
    'umaInsightTopCategory': 'Больше всего вы потратили на {category}: {amount}, это около {share}% от проанализированного общего объёма {total}.',
    ...
    'onbStep1Subtitle': 'Сначала выберите язык. Позже его всегда можно поменять в настройках.',
    'onbStep2Title': 'Сделайте под себя',
    'onbStep2Subtitle': 'Выберите палитру и настроение бренда. Тему можно изменить и позже.',
    'onbStep3Title': 'Добавьте свои данные',
    'onbStep3Subtitle': 'Vera не подключается к вашему банку — вы приносите выписки, а Vera помогает их понять. Начните сейчас или добавьте позже.',
    ...
    'onbPalettePreviewBody': 'Ваш выбор сразу меняет карточки, AI-акценты и общее настроение интерфейса.',
    'onbBlankCanvasTitle': 'Начать с пустого экрана тоже нормально',
    'onbBlankCanvasBody': 'Если хотите, продолжайте сейчас без импорта; Vera подскажет вам и первый шаг уже внутри приложения.',
    'goalEmptyPrompt': 'Вы ещё не задали цель — нажмите, чтобы добавить.',
    'goalEmptyHint': 'Сначала задайте цель, и Uma подскажет, сколько стоит откладывать каждый месяц в вашем ритме.',
    ...
    'noTransactionsBody': 'Добавьте первую операцию, импортируйте выписку или отсканируйте чек.',
    'addHoldingTitle': 'Добавить актив в портфель',
    'addHoldingSubtitle': 'Выберите один из классов ниже и введите стоимость в TL. Uma автоматически пересчитает веса.',
    ...
    'noWealthActionsBody': 'Когда вы включите автономный режим, здесь появятся рекомендации и ребалансировки.',
    'startPortfolioTitle': 'Начните собирать портфель',
    'startPortfolioBody': 'Добавьте акции, золото, наличные или криптоактивы. Uma будет следить за отклонениями и писать сюда советы.',
    ...
    'noSubscriptionsDetectedBody': 'Импортируйте выписку или отсканируйте чек — Vera автоматически найдёт регулярные платежи.',
    ...
    'subsAlertSavingsMessageEmpty': 'Подписки пока не найдены. Импортируйте выписку или добавьте подписку вручную, и Vera начнёт её отслеживать.',
    'subsAlertSavingsMessageActive': 'Vera нашла подписки, которые можно заморозить или перевести на более дешёвый план без ущерба для вашего ритма.',
    ...
    'subsAlertPriceMessageSome': 'Один или несколько планов подорожали по сравнению с прошлым месяцем.',
    ...
    'subsAlertUnusedMessageSome': 'По вашему недавнему паттерну активности {n} планов выглядят малоиспользуемыми.',
    'subsAlertUnusedMetric': 'ПУСТО',
    'subsInsightEmpty': 'Vera анализирует ваши выписки или чеки и собирает подписки здесь.',
    'subsInsightHealthy': 'В этом месяце подписки выглядят здоровыми. Срочной утечки экономии не видно.',
    'subsInsightNeedsAttention': 'У вас есть {n} подписок, которые стоит проверить. Vera оценивает, что вы можете вернуть около {amount} в месяц, если заморозите неиспользуемые и понизите более дорогие планы.',
    ...
    'subsRecommendationDetected': 'Vera обнаружила эту подписку в импортированной выписке. Вы всё ещё ей пользуетесь?',
    ...
    'proactiveBillBody': '{amount} · осталось {days} дн. Попросить Uma подготовить оплату?',
    ...
    'proactiveUnusedBody': 'Последнее использование: {last}. Это может быть тихой утечкой денег.',
    'proactiveUnusedCta': 'Открыть подписки',
    'proactiveHealthyTitle': 'Сейчас нет критических сигналов',
    'proactiveHealthyBody': 'Сейчас не видно ни срочного счёта, ни скачка цены, ни явно бесполезной подписки. Если хотите, Uma всё равно может быстро оценить этот период.',
    'proactiveHealthyCta': 'Попросить Uma оценить',
    'proactiveEmptyTitle': 'Vera пока не нашла сигнал для отслеживания',
    'proactiveEmptyBody': 'После импорта выписки Vera будет показывать здесь ближайшие платежи, подписки и небольшие риски.',
    ...
    'signupTerms':
        'Я принимаю условия Vera и политику локальных демо-данных.',
